...
import { arch, isLinux, isPortable, isWindows, platform } from '@main/utils/systeminfo'
...
import { getDeviceId } from '@main/utils/tool'
 ...
 * 注册主进程 IPC 处理函数
 *
 * @param mainWindow 主窗口实例，用于某些业务需要与主窗交互的场景
 * @param app        Electron.App 实例，用于应用级控制（退出、重启、代理配置等）
 *
 * 说明：
 * - 本函数将所有 IPC_CHANNEL 对应的 handler 统一集中注册
 * - 便于在一个地方查看/维护所有主进程暴露给渲染进程的能力
 */
export function registerIpc(mainWindow: BrowserWindow, app: Electron.App) {
  ...
  ipcMain.handle(
    ...
    async (_, type: IProxyType, proxy?: string, bypass?: string) => {
  ...
  ipcMain.handle(IPC_CHANNEL.APP_PROXY_SYSTEM, () => {
    ...
    if (platform === 'darwin')
      shell.openExternal('x-apple.systempreferences:com.apple.preference.network?Proxies')
  ...
  // device
  ipcMain.handle(IPC_CHANNEL.DEVICE_ID, (_, time: string) => {
    return getDeviceId(time)
  })
  ...
  // file
  ipcMain.handle(
    IPC_CHANNEL.FILE_SELECT_FOLDER_DIALOG,
    (_, options?: Electron.OpenDialogOptions) => {
  ...
  ipcMain.handle(
    IPC_CHANNEL.FS_DIR_READ,
    async (_, path: string, depth: number = 0, exclude?, include?) => {
  ...
  // shortcut
  ipcMain.handle(
    IPC_CHANNEL.SHORTCUTS_IS_REGISTERD,
    (_, type: IShortcutType, id: string, winName?: string) => {
  ...
  ipcMain.handle(
    ...
    (_, id: string, config: IShortcutConfig, force: boolean) => {
      return shortcutService.register(id, config, force)
    }
  )

  ipcMain.handle(
    IPC_CHANNEL.SHORTCUT_UNREGISTER,
    (_, type: IShortcutType, id: string, winName?: string) => {
  ...
  // window
  ipcMain.handle(
    IPC_CHANNEL.WINDOW_SIZE,
    (event: Electron.IpcMainInvokeEvent, width: number, height: number) => {
      const win = BrowserWindow.fromWebContents(event.sender)!
