...
export class ShortcutService {
  ...
  public registerLocal(id: string, config: IShortcutConfig, force: boolean = false): boolean {
    ...
    if (isNil(id) || type !== SHORTCUT_TYPE.LOCAL || isNil(shortcutRaw) || isNil(winName))
      return false
    ...
    if (localShortcut.isRegistered(win, shortcut)) {
      ...
      const entry = [...this.shortcutPool.entries()].find(
        ([, conf]) =>
          conf.type === SHORTCUT_TYPE.LOCAL &&
          conf.shortcut === shortcut &&
          conf.winName === winName
      )

      if (!entry) {
        logger.warn(
          `Local shortcut occupied by system or other app: ${shortcut} for window: ${winName}`
        )
        return false
      } // unregister the existing one
  ...
  public register(id: string, config: IShortcutConfig, force: boolean = false): boolean {
    if (
      ...
      (config.type === SHORTCUT_TYPE.LOCAL &&
        !(isString(config.winName) && !isStrEmpty(config.winName)))
    ) {
      return false
    }
