...
export default class AppUpdater {
  ...
  public async checkForUpdates() {
    ...
    try {
      ...
      logger.info(
        `[Updater] available=${isUpdateAvailable}, current=${currentVersion}, latest=${lastVersion}`
      )
      ...
      const releaseNoteMarkdown =
        isString(releaseNotes) && !isStrEmpty(releaseNotes)
          ? turndownService.turndown(releaseNotes)
          : ''
