...
import apiApp from '@renderer/api/modules/app'
import { systemRoutes as systemRoutesRaw } from '@renderer/router/routes'

export const useRouteStore = defineStore(
  // 唯一ID
  'route',
  (): {
    isGenerate: Ref<boolean>
    routesRaw: Ref<Route.recordMainRaw[]>
    currentRemoveRoutes: Ref<(() => void)[]>
    routes: ComputedRef<RouteRecordRaw[]>
    systemRoutes: ComputedRef<RouteRecordRaw[]>
    getRouteMatchedByPath: (path: string) => RouteRecordRaw[]
    generateRoutesAtFront: (asyncRoutes: Route.recordMainRaw[]) => void
    generateRoutesAtBack: () => Promise<void>
    generateRoutesAtFilesystem: (asyncRoutes: RouteRecordRaw[]) => void
    setCurrentRemoveRoutes: (routes: (() => void)[]) => void
    removeRoutes: () => void
  } => {
    ...
    function formatBackRoutes(routes: any, views = import.meta.glob('../../views/**/*.vue')): Route.recordMainRaw[] {
      return routes.map((route: any) => {
        switch (route.component) {
          case 'Layout':
            route.component = () => import('@renderer/layouts/index.vue')
            break
          default:
            if (route.component) {
