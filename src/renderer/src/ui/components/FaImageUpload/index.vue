...
function onSelectFile(e: Event) {
  ...
  const remain
    = props.max === 0 ? selectedFiles.length : props.max - images.value.length
  ...
  for (const file of selectedFiles.slice(0, remain)) {
    ...
    if (props.size > 0) {
      if (file.size > props.size) {
        toast.error(
          t('sizeError', { size: filesize(props.size, { standard: 'jedec' }) }),
          {
...
function onMove(index: number, direction: 'forward' | 'backward') {
  if (direction === 'forward' && index !== 0) {
    images.value[index] = images.value.splice(
      index - 1,
      1,
      images.value[index],
    )[0]
      ...
      >
        ...
        >
          ...
          <div
            ...
            :class="
              cn('flex-center cursor-pointer opacity-60 transition-all', {
