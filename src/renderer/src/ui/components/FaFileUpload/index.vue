...
function onSelectFile(files: FileList | File[] | null) {
  ...
  // 数量限制
  const remain
    = props.max > 0 ? props.max - fileList.value.length : selectedFiles.length
  ...
  axios({
    ...
    onUploadProgress: (progressEvent) => {
      if (progressEvent.total) {
        fileList.value[currentFileIndex].progress = Math.round(
...
<template>
  <div class="space-y-2">
    <button
      ...
      :class="{
        ...
        'opacity-50 cursor-not-allowed':
          props.disabled || (props.max > 0 && fileList.length >= props.max),
      }"
      :disabled="
        props.disabled || (props.max > 0 && fileList.length >= props.max)
      "
    ...
    >
      <slot>
        <FaIcon
          ...
          class="mb-2 text-2xl text-card-foreground/50"
        />
        <div class="text-sm text-card-foreground/70">
          {{ t("uploadDragTips")
          }}<span class="cursor-pointer text-primary font-bold">{{
            t("uploadClickTips")
          }}</span>
        </div>
      </slot>
    ...
    <div
      v-if="!props.hideTips && !props.disabled"
    ...
    >
      <div
        ...
        :class="[item.status === 'error' ? 'border-red-500 bg-red-500/10' : '']"
