  ...
  <Dialog :modal="false" :open="isOpen" @update:open="updateOpen">
    <DialogContent
      ...
      <DialogHeader
        ...
        <slot name="header">
          <DialogTitle class="flex-center gap-x-2 md:justify-start"
          ...
          <DialogDescription class="empty:hidden"
      ...
      <div v-if="!!slots.default"
        :class="cn('relative flex-1 min-h-40 p-4 overflow-y-auto', props.contentClass)">
        <slot />
      </div>
      <div v-show="props.loading"
        class="absolute inset-0 z-1000 size-full flex-center bg-popover/75">
        <FaIcon name="i-line-md:loading-twotone-loop"
          class="size-10" />
      </div>
      <DialogFooter
        ...
        <slot name="footer">
          <FaButton v-if="showCancelButton" variant="outline"
            @click="onCancel">
            {{ cancelButtonText }}
          </FaButton>
          <FaButton v-if="showConfirmButton"
            :disabled="confirmButtonDisabled"
            :loading="confirmButtonLoading || isConfirmButtonLoading"
            @click="onConfirm">
            {{ confirmButtonText }}
          </FaButton>
        </slot>
