  ...
  <div
    ...
    <div :class="vertical ? 'flex flex-1 gap-4' : 'contents'">
      <div
        ...
        :class="cn('h-full flex-none overflow-auto border rounded-lg bg-background p-4 transition-background-color-300', props.leftSideClass)"
        :style="{
      ...
      <div class="flex flex-1 flex-col gap-4">
        <Teleport :to="`#${topSideTeleportId}`" defer
          :disabled="!vertical">
          <div
            v-if="enabledTopSide" v-show="topSideVisiable"
            :class="cn('w-full border rounded-lg bg-background p-4 transition-background-color-300', props.topSideClass)"
            :style="{
        ...
        <div class="relative h-full w-full flex flex-1">
          <div v-if="enabledLeftSide && !hideLeftSideToggle"
            class="group left-side-icon absolute top-1/2 z-1 h-6 w-6 flex-center cursor-pointer border-l border-t rounded-1/2 bg-background -left-3 -translate-y-1/2 -rotate-45"
          ...
          <div v-if="enabledRightSide && !hideRightSideToggle"
            class="group right-side-icon absolute top-1/2 z-1 h-6 w-6 flex-center cursor-pointer border-b border-r rounded-1/2 bg-background -right-3 -translate-y-1/2 -rotate-45"
            @click="rightSideVisiable = !rightSideVisiable">
            <FaIcon
              :name="rightSideVisiable ? 'i-ep:caret-right' : 'i-ep:caret-left'"
              class="rotate-45 op-30 transition-opacity group-hover-op-100" />
          </div>
          <div v-if="enabledTopSide && !hideTopSideToggle"
            class="group top-side-icon absolute left-1/2 z-1 h-6 w-6 flex-center rotate-45 cursor-pointer border-l border-t rounded-1/2 bg-background -top-3 -translate-x-1/2"
            @click="topSideVisiable = !topSideVisiable">
            <FaIcon
              :name="topSideVisiable ? 'i-ep:caret-top' : 'i-ep:caret-bottom'"
              class="op-30 transition-opacity -rotate-45 group-hover-op-100" />
          </div>
          <div v-if="enabledBottomSide && !hideBottomSideToggle"
            class="group bottom-side-icon absolute left-1/2 z-1 h-6 w-6 flex-center rotate-45 cursor-pointer border-b border-r rounded-1/2 bg-background -bottom-3 -translate-x-1/2"
            @click="bottomSideVisiable = !bottomSideVisiable">
            <FaIcon
              :name="bottomSideVisiable ? 'i-ep:caret-bottom' : 'i-ep:caret-top'"
              class="op-30 transition-opacity -rotate-45 group-hover-op-100" />
          </div>
          <div
            :class="cn('absolute inset-0 h-full w-full overflow-auto border rounded-lg bg-background p-4 transition-background-color-300', props.defaultClass)">
            <slot name="default" />
          </div>
        </div>
        <Teleport :to="`#${bottomSideTeleportId}`" defer
          :disabled="!vertical">
          <div
            v-if="enabledBottomSide" v-show="bottomSideVisiable"
            :class="cn('w-full border rounded-lg bg-background p-4 transition-background-color-300', props.bottomSideClass)"
            :style="{
      ...
      <div
        ...
        :class="cn('h-full flex-none overflow-auto border rounded-lg bg-background p-4 transition-background-color-300', props.rightSideClass)"
        :style="{
