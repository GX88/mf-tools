...
import { icons } from '@renderer/iconify'
  ...
  <FaPopover v-model:open="open" :collision-padding="5" class="p-0">
    <slot :icon="value">
      <FaButton variant="outline" size="icon"
        ...
        <FaIcon :name="value !== '' ? value : 'i-whh:googleplusold'"
          class="size-4" :class="{ 'opacity-20': value === '' }" />
      </FaButton>
    </slot>
    <template #panel>
      <div class="h-500px w-600px flex-center of-hidden rounded-md">
        <FaScrollArea :scrollbar="false" mask
          class="h-full w-150px shrink-0 border-r">
          <div v-for="item in icons" :key="item.prefix"
            ...
            <div
              class="text-base text-accent-foreground leading-tight">
              {{ item.info.name }}
            </div>
        ...
        <div class="h-full flex-col-center flex-1 p-6">
          ...
          <div
            class="grid grid-cols-7 grid-rows-7 my-2 w-full flex-1 place-items-center gap-1">
            <FaButton variant="outline" size="icon" class="h-12 w-12"
              @click="removeIcon">
              <FaIcon name="i-ep:delete" class="size-6 opacity-20" />
            </FaButton>
            <FaButton v-for="(icon, index) in currentIconList"
              :key="index" variant="ghost" size="icon"
              class="h-12 w-12 [&_svg]:size-inherit"
              @click="chooseIcon(`${activeName}:${icon}`)">
              <FaIcon :name="`${activeName}:${icon}`"
                class="size-6" />
            </FaButton>
          </div>
          <FaPagination :page="pagination.page"
            :size="pagination.pageSize" :total="iconList.length"
