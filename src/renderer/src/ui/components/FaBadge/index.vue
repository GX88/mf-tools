  ...
  <div :class="cn('relative inline-flex', props.class)">
    ...
    <Transition v-bind="transitionClass">
      <div v-if="show" class="absolute h-full w-full">
        <div v-if="value === true"
          :class="badgeDotVariant({ variant })" />
        <Badge v-else :variant
          class="absolute start-[50%] top-0 z-20 whitespace-nowrap px-1.5 py-0 ring-1 ring-primary-foreground -translate-y-[50%] hover:bg-none!">
          {{ value }}
        </Badge>
      </div>
    </Transition>
