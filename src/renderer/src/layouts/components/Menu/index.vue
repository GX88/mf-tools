...
const openedMenus = ref<MenuInjection['openedMenus']>([...new Set([...props.alwaysOpeneds.slice(0), ...props.defaultOpeneds.slice(0)])])
  ...
  <div
    ...
    <template v-for="item in menu" :key="item.path ?? getUseId(item)">
      <template v-if="item.meta?.menu !== false">
        <SubMenu v-if="item.children?.length" :menu="item"
          :unique-key="[item.path ?? (item.children.every(item => item.meta?.menu === false) ? item.children[0].path! : getUseId(item))]" />
        <Item v-else :item="item"
          :unique-key="[item.path ?? (item.children?.every(item => item.meta?.menu === false) ? item.children[0].path! : getUseId(item))]"
          @click="handleMenuItemClick(item.path ?? (item.children?.every(item => item.meta?.menu === false) ? item.children[0].path! : getUseId(item)))" />
      </template>
    </template>
  </div>
</template>
