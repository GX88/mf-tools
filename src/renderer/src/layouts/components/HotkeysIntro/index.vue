  ...
  <FaModal v-model="isShow" :title="t('title')" :footer="false">
    <div class="px-4">
      <div class="grid gap-4 sm-grid-cols-2">
          ...
          <ul class="list-none ps-2 pt-2 text-sm">
            <li class="flex-baseline gap-2 py-1">
              <div class="flex-shrink-0 space-x-1">
                <FaKbd>{{ settingsStore.os === 'mac' ? '⌘' : 'Ctrl' }}
                </FaKbd>
                <FaKbd>I</FaKbd>
              </div>
              {{ t('global.system') }}
        ...
        <div
          v-if="settingsStore.settings.menu.enableHotkeys && ['side', 'head'].includes(settingsStore.settings.menu.mode)">
          <h2 class="m-0 text-lg font-bold">
            {{ t('nav.root') }}
          </h2>
          <ul class="list-none ps-2 pt-2 text-sm">
            <li class="flex-baseline gap-2 py-1">
              <div class="flex-shrink-0 space-x-1">
                <FaKbd>{{ settingsStore.os === 'mac' ? '⌥' : 'Alt' }}
                </FaKbd>
                <FaKbd>`</FaKbd>
              </div>
              {{ t('nav.next') }}
