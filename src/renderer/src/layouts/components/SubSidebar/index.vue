  ...
  <Transition name="sub-sidebar">
    <div
      ...
      <FaScrollArea :scrollbar="false" mask
        ...
        class="flex-1 overscroll-contain">
      ...
      <div
        v-if="settingsStore.mode === 'pc' && settingsStore.settings.menu.enableSubMenuCollapseButton"
        class="relative flex items-center px-4 py-3" :class="{
          'justify-center': isCollapse,
          'justify-between': !isCollapse,
        }">
        <FaButton v-show="!isCollapse" variant="secondary" size="icon"
          class="h-8 w-8"
          @click="settingsStore.toggleSidebarAutoCollapse()">
          <FaIcon
            :name="settingsStore.settings.menu.subMenuAutoCollapse ? 'i-lucide:pin-off' : 'i-lucide:pin'"
            class="size-4" />
        </FaButton>
        <FaButton
          v-show="!settingsStore.settings.menu.subMenuAutoCollapse"
          variant="secondary" size="icon" class="h-8 w-8 transition"
      ...
      <div v-if="settingsStore.settings.menu.mode === 'single'"
        class="flex-center px-4 pb-3">
        <AccountButton :only-avatar="isCollapse"
          dropdown-align="center"
          :dropdown-side="isCollapse ? (settingsStore.settings.app.direction === 'rtl' ? 'left' : 'right') : 'top'"
          button-variant="secondary"
          :class="{ 'w-full': !isCollapse }" />
      </div>
