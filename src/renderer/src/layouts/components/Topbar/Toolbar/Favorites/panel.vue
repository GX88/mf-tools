    ...
    <FaScrollArea class="max-h-220px">
      ...
      >
        <div
          ...
          class="group draggable-item relative w-[calc(50%-0.25rem)] flex cursor-pointer items-center gap-1 rounded-lg px-2 py-2 ring-1 ring-border ring-inset hover-bg-accent"
          @click="handleChoose(item.fullPath)"
        >
          <FaIcon v-if="item.icon" :name="item.icon" class="size-4" />
          <div
            class="flex-1 truncate pe-4 text-sm group-hover:[mask-image:linear-gradient(90deg,#000_calc(100%-80px),transparent)]"
          >
            {{ generateI18nTitle(item.title) }}
          </div>
          <div
            class="absolute right-2 hidden h-5 w-5 rounded-full text-secondary-foreground group-hover:flex-center hover:(bg-secondary ring-1)"
          >
            <FaIcon
              name="i-ep:delete"
              class="size-3"
      ...
      <div v-else class="flex-col-center py-6 text-secondary-foreground">
        ...
        <p
          ...
          class="m-0 flex-center text-sm op-75"
        >
          {{ t("addToFavorites") }}
          <FaIcon
            name="i-mdi:star-plus-outline"
            class="mx-1 size-5 text-secondary-foreground"
          />
          {{ t("addCurrentPage") }}
        </p>
      </div>
