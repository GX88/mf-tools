...
const searchResultItemRef = useTemplateRef<HTMLElement[]>(
  'searchResultItemRef',
)
...
const resultList = computed(() => {
  ...
  result = sourceList.value.filter((item) => {
    ...
    if (searchInput.value !== '') {
      ...
      if (
        routeStore.getRouteMatchedByPath(item.path).some((b) => {
          return (
            generateI18nTitle(b.meta?.title)
              .toString()
              .includes(searchInput.value)
              || (match(
                generateI18nTitle(b.meta?.title).toString(),
                searchInput.value,
                { continuous: true },
              )?.length ?? 0) > 0
          )
        ...
        >
          ...
          >
            ...
            <div class="flex flex-1 flex-col gap-1 truncate border-s px-4 py-3">
              ...
              >
                <BreadcrumbItem
                  v-for="(bc, bcIndex) in routeStore.getRouteMatchedByPath(
                    item.path,
                  )"
                  :key="bcIndex"
                  class="text-xs"
