...
import Breadcrumb from '@renderer/layouts/components/Breadcrumb/index.vue'
import BreadcrumbItem from '@renderer/layouts/components/Breadcrumb/item.vue'
import { resolveRoutePath } from '@renderer/utils'
...
<template>
  <FaModal v-model="isShow" border
    :footer="settingsStore.mode === 'pc'" :closable="false"
    class="w-full lg-max-w-2xl"
    content-class="flex flex-col p-0 min-h-auto" header-class="p-0"
    footer-class="p-0" @opened="searchInputFocused = true">
    <template #header>
      <div class="h-12 flex flex-shrink-0 items-center">
        <div class="h-full w-14 flex-center">
          <FaIcon name="i-ri:search-line"
            class="size-4 text-foreground/30" />
        </div>
        <input ref="searchInputRef" v-model="searchInput"
          :placeholder="t('input')"
          ...
          @keydown.up.prevent="keyUp" @keydown.down.prevent="keyDown"
          @keydown.enter.prevent="keyEnter">
        <div v-if="settingsStore.mode === 'mobile'"
          class="h-full w-14 flex-center border-s">
          <FaIcon name="i-carbon:close" class="size-4"
            @click="isShow = false" />
        </div>
      </div>
    </template>
    <template #footer>
      <div class="w-full flex justify-between px-4 py-3">
        ...
        <div v-if="settingsStore.settings.navSearch.enableHotkeys"
    ...
    <FaScrollArea ref="searchResultRef">
      <template v-if="resultList.length > 0">
        <div v-for="(item, index) in resultList"
          ref="searchResultItemRef" :key="item.path" class="p-4"
          :data-index="index"
          @click="pageJump(item.path, item.query, item.link)"
          @mouseover="actived = index">
          <a class="flex cursor-pointer items-center border rounded-lg"
            ...
            <div
              class="flex flex-1 flex-col gap-1 truncate border-s px-4 py-3">
              <div class="truncate text-start text-base font-bold">
                {{ generateI18nTitle(item.title) }}
              </div>
              <Breadcrumb
                v-if="routeStore.getRouteMatchedByPath(item.path).length"
                class="truncate">
                <BreadcrumbItem
                  v-for="(bc, bcIndex) in routeStore.getRouteMatchedByPath(item.path)"
                  :key="bcIndex" class="text-xs">
      ...
      <template v-else-if="searchInput === ''">
        <div
          class="h-full flex-col-center py-6 text-secondary-foreground/50">
