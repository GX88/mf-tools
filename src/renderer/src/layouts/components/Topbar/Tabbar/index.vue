...
const isShowMoreAction = computed(
  () =>
    ...
    && (tabbar.checkCloseOtherSide()
      || tabbar.checkCloseLeftSide()
      || tabbar.checkCloseRightSide()),
)
...
onMounted(() => {
  tabSortable = new Sortable(tabContainerRef.value?.$el, {
    ...
    onMove: (e) => {
      return e.dragged.classList.contains('pinned')
        ? e.related.classList.contains('pinned')
        : !e.related.classList.contains('pinned')
    },
    onUpdate: (e) => {
...
watch(
  [() => settingsStore.mode, () => settingsStore.settings.app.direction],
  (val) => {
    nextTick(() => {
  ...
  (val) => {
    if (settingsStore.settings.tabbar.enable) {
      ...
      nextTick(() => {
        ...
        if (index !== -1) {
          const tabEl = tabRef.value?.find(
            item => Number.parseInt(item.dataset.index!) === index,
          )
...
function tabbarScrollTip() {
  if (
    ...
    > (tabsRef.value?.ref?.$el.clientWidth ?? 0)
    && !localStorage.has('tabbarScrollTip')
  ) {
    ...
    [
      {
        ...
        disabled:
          routeItem.fullPath === settingsStore.settings.home.fullPath
          || routeItem.isPermanent,
        handle: () =>
          routeItem.isPin
            ? tabbarStore.unPin(routeItem.tabId)
            : tabbarStore.pin(routeItem.tabId),
...
function getVisibleTabIndex(arrayIndex: number) {
  return (
    visibleTabIndex.value.findIndex(
      visibleTab => visibleTab === arrayIndex,
    ) ?? -1
  )
}
watch(
        ...
        switch (handle.key) {
          ...
          case 'alt+9': {
            ...
            if (visibleTabIndex.value[number - 1] !== undefined) {
              router.push(
                tabbarStore.list[visibleTabIndex.value[number - 1]].fullPath,
              )
            }
            break
          }
    ...
    <div class="tabbar-container">
      <FaScrollArea
        ...
        :class="{
          ...
          [`tabs-${settingsStore.settings.tabbar.style}`]:
            settingsStore.settings.tabbar.style !== '',
        }"
      >
        <TransitionGroup
          ref="tabContainerRef"
        ...
        >
          <div
            ...
            :key="
              element.tabMerge === 'routeName' && element.routeName
                ? element.routeName
                : element.tabId
            "
            ref="tabRef"
            ...
            @click="!isAnimating && router.push(element.fullPath)"
            @dblclick="!isAnimating && onTabbarDblclick(element)"
          >
                ...
                <FaTooltip :delay="1000" side="bottom">
                  <div class="tab-content">
                    <div :key="element.tabId" class="title">
                      <FaIcon
                        v-if="
                          settingsStore.settings.tabbar.enableIcon
                            && iconName(
                      ...
                      {{
                        ...
                        )?.title || generateI18nTitle(element.title)
                      }}
                    </div>
                    <div
                    ...
                    <div
                      v-else-if="
                        !element.isPermanent && tabbarStore.list.length > 1
                      "
                      class="action-icon"
                    ...
                    <div
                      v-show="
                        keys.alt
                          && getVisibleTabIndex(index) >= 0
                          && getVisibleTabIndex(index) < 9
                      "
                      class="hotkey-number"
                    >
        ...
        .tab {
          ...
          * {
          ...
          .tab-content {
            ...
            .action-icon {
              --uno: transition absolute inset-e-2 top-1/2 -translate-y-1/2
                rounded-full z-10 w-5 h-5 flex-center text-xs
                "text-[var(--g-tabbar-tab-color)]" hover: (border bg-secondary);
            }

            .hotkey-number {
              --uno: border bg-secondary absolute inset-e-2
                top-1/2 -translate-y-1/2 rounded-full z-10 w-5 h-5 flex-center
                text-xs "text-[var(--g-tabbar-tab-color)]";
            }
