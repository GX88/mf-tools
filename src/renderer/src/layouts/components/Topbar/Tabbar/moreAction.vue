...
const resultList = computed(() => {
  ...
  result = tabbarStore.list.filter((item) => {
    ...
    if (
      ...
      || (match(generateI18nTitle(item.title).toString(), searchInput.value, {
        continuous: true,
      })?.length ?? 0) > 0
    ) {
      flag = true
      ...
      [
        {
          ...
          icon: 'i-mdi:close',
          handle: () => tabbar.closeOtherSide(),
          disabled: !tabbar.checkCloseOtherSide(),
        },
        {
          label: t('closeLeftSide'),
          icon: 'i-mdi:arrow-expand-left',
          handle: () => tabbar.closeLeftSide(),
          disabled: !tabbar.checkCloseLeftSide(),
        },
        {
          label: t('closeRightSide'),
          icon: 'i-mdi:arrow-expand-right',
          handle: () => tabbar.closeRightSide(),
          disabled: !tabbar.checkCloseRightSide(),
        },
      ],
    ]"
    ...
    <template #footer>
      <div class="max-h-80 w-40 flex flex-col gap-2">
        <input
          ...
          class="h-8 border border-input rounded-md bg-background px-3 py-2 text-sm placeholder:text-muted-foreground focus-visible:outline-none"
        >
        <FaScrollArea
          :scrollbar="false"
          mask
          gradient-color="hsl(var(--popover))"
          class="overscroll-contain"
        >
          <div
            ref="dropdownTabContainerRef"
            class="tabs space-y-1"
            :class="{ dragging: isDragging }"
          >
            <div
              v-for="element in resultList"
              :key="
                element.tabMerge === 'routeName' && element.routeName
                  ? element.routeName
                  : element.tabId
              "
              class="relative h-8 rounded-md px-2 tab"
              :class="{
            ...
            >
              <div
                ...
                "
                @click="handleClick(element.fullPath)"
              >
                <FaIcon
                  v-if="
              ...
              <div
                v-else-if="!element.isPermanent && resultList.length > 1"
                class="action-icon"
...
.tabs {
  ...
  .tab {
    ...
    * {
    ...
    .action-icon {
      --uno: transition absolute inset-e-2 top-1/2 -translate-y-1/2 rounded-full
        z-10 w-5 h-5 flex-center text-xs "text-[var(--g-tabbar-tab-color)]" op-0
