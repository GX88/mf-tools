...
<template>
  <div
    ...
    <router-link v-slot="{ href, navigate }" custom
      :to="(uniqueKey.at(-1) ?? '') + (item.meta?.query ? `?${qs.stringify(item.meta?.query)}` : '')">
      <FaTooltip :disabled="level !== 0 || subMenu"
        :text="generateI18nTitle(item.meta?.title)"
        ...
        <component
          ...
          <div
            ...
            <FaIcon v-if="icon" :name="icon"
              class="menu-item-container-icon" />
            <span
