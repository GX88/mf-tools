  ...
  <div class="min-h-500px w-full flex-col-stretch-center p-12">
    ...
    <div v-show="type === 'default'">
      <form @submit="onSubmit">
          ...
          <FormItem class="relative pb-6 space-y-0">
            <FormControl>
              <FaInput type="password"
                :placeholder="t('form.password')" class="w-full"
                :class="errors.length > 0 && 'border-destructive'"
                v-bind="componentField" />
            </FormControl>
            <Transition enter-active-class="transition-opacity"
        ...
        <div class="mb-4 flex-center-between">
          <div class="flex-center-start">
            <FormField v-slot="{ componentField }" type="checkbox"
              name="remember">
              <FormItem>
                <FormControl>
        ...
        <FaButton :loading="loading" size="lg" class="w-full"
          type="submit">
          {{ t('form.login') }}
        </FaButton>
        <div class="mt-4 flex-center gap-2 text-sm">
          <span
            class="text-secondary-foreground op-50">{{ t('noAccount') }}</span>
          <FaButton variant="link" class="h-auto p-0" type="button"
      ...
      <div class="mt-4 text-center -mb-4">
        ...
        <div class="space-x-2">
          <FaButton variant="default" size="sm" plain
            @click="testAccount('admin')">
            admin
          </FaButton>
          <FaButton variant="outline" size="sm" plain
    ...
    <div v-show="type === 'qrcode'">
      <div class="flex-col-center">
        <img src="https://s2.loli.net/2024/04/26/GsahtuIZ9XOg5jr.png"
