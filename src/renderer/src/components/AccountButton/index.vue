...
<template>
  <FaDropdown
    ...
    <template #header>
      <div class="space-y-2">
        ...
        <div class="flex-center-start gap-2">
          <FaAvatar :src="userStore.avatar"
            :fallback="userStore.account.slice(0, 2)"
            shape="square" />
          <div class="space-y-1">
            ...
            <div
              class="text-xs text-secondary-foreground/50 font-normal">
              [ 这里可以显示邮箱 ]
            </div>
          </div>
        </div>
    ...
    <FaButton :variant="buttonVariant" :class="cn('flex-center gap-1 p-2', {
      'size-8 p-1': onlyAvatar,
    }, props.class)">
      <FaAvatar :src="userStore.avatar"
        :class="cn('size-6', { 'size-full': onlyAvatar })">
        <FaIcon name="i-carbon:user-avatar-filled"
          class="size-6 text-secondary-foreground/50" />
      </FaAvatar>
      <div v-if="!onlyAvatar"
        class="min-w-0 flex-center-between flex-1 gap-2">
  ...
  <FaModal v-model="isProfileShow" align-center :header="false"
    :footer="false" :close-on-click-overlay="false"
    :close-on-press-escape="false"
    class="h-500px max-w-xl overflow-hidden"
    content-class="min-h-full p-0 flex">
    <Profile />
  </FaModal>
  <FaModal v-model="isFeedbackShow" :title="t('feedback')"
    :footer="false" class="max-w-[800px]">
